-- 🗄️ HABESHLIVE - COMPLETE SUPABASE DATABASE SCHEMA
-- Extending profiles table and creating new tables
...
CREATE POLICY "Users can view own history" ON public.respect_score_history 
...
CREATE POLICY "Users can view own sessions" ON public.video_sessions 
  FOR SELECT USING (auth.uid() = user1_id OR auth.uid() = user2_id);

CREATE POLICY "Users can update own sessions" ON public.video_sessions 
...
CREATE POLICY "Users can update own status" ON public.online_users 
...
-- Update respect tier based on score
...
-- Auto-ban users with score < 50
CREATE OR REPLACE FUNCTION auto_ban_low_score()
RETURNS TRIGGER AS $$
BEGIN
  IF NEW.respect_score < 50 AND OLD.respect_score >= 50 THEN
    NEW.banned := true;
...
CREATE TRIGGER auto_ban_trigger
  BEFORE UPDATE OF respect_score ON public.profiles
